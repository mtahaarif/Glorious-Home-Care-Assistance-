...
// 1. Tell Next.js to statically build a page for all article slugs
...
export async function generateMetadata({ 
  ...
  // SEO Fix: Keep title under 60 characters and use the specific article title.
  const rawTitle = `${article.title} | Glorious Home Care`;
  const optimizedTitle = rawTitle.length > 60 ? rawTitle.substring(0, 57) + "..." : rawTitle;

  const pageUrl = `https://www.glorioushomecareassistance.com/resources/${article.slug}`;

  return {
    // ✅ SEO FIX: Use 'absolute' to bypass the long root layout template and prevent "Title too long" warnings
    title: {
